...
const { selectors: selectorReferences } = require("../stylelint-internal")
...
function isStandardSyntaxTypeSelector(node) {
    ...
    if (node.parent && node.parent.parent) {
        ...
        if (targetParent.value && targetParent.type === "pseudo") {
            ...
            if (
                selectorReferences.aNPlusBNotationPseudoClasses.has(
                    normalizedParentName,
                ) ||
                selectorReferences.aNPlusBOfSNotationPseudoClasses.has(
                    normalizedParentName,
                ) ||
                selectorReferences.linguisticPseudoClasses.has(
                    normalizedParentName,
                ) ||
                selectorReferences.shadowTreePseudoElements.has(
                    normalizedParentName,
                )
            ) {
                return false
