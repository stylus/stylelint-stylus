...
function restoreReplacedSelector(orgNode, replaceSelectorContext) {
    ...
    if (isPostCSSSPContainer(node)) {
        const nodes = node.nodes
            .map(childNode =>
                restoreReplacedSelector(childNode, replaceSelectorContext)
            )
            .filter(n => Boolean(n))
        node.nodes = nodes
    }
 ...
 * @param {PostCSSSPNode} node
 * @param {ReplaceInfo[]} replaces
 * @param {string | null} cssText
 * @returns {PostCSSSPNode | null}
 */
