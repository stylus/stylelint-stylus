...
const rule = (primary, secondaryOptions = {}, context = {}) => {
    return (root, result) => {
        ...
        const legibleExpectation = (level) => {
            ...
            const quantifiedWarningWord =
                count === 1 ? warningWord : `${warningWord}s`
        ...
        root.walk((node) => {
            ...
            const after =
                typeof node.raws.after === "string" ? node.raws.after : ""
            ...
            if (
                (lastIndexOfNewline !== -1 ||
                    (isFirstChild &&
                        (!getDocument(parent) ||
                            (parent.raws.codeBefore &&
                                parent.raws.codeBefore.endsWith("\n"))))) &&
                before.slice(lastIndexOfNewline + 1) !==
            ...
            const closingBraceLevel = indentClosingBrace
                ? nodeLevel + 1
                : nodeLevel
            const expectedClosingBraceIndentation =
                indentChar.repeat(closingBraceLevel)
            ...
            if (
                ...
                if (context.fix) {
                    ...
                    report({
                        message: messages.expected(
                            legibleExpectation(closingBraceLevel),
                        ),
                        node,
        ...
        function checkValue(decl, declLevel) {
            ...
            const valueLevel = optionsMatches(
                secondaryOptions,
                "except",
                "value",
            )
        ...
        function checkMultilineBit(source, newlineIndentLevel, node) {
            ...
            const ignoreInsideParans = optionsMatches(
                secondaryOptions,
                "ignore",
                "inside-parens",
            )
            ...
            if (fixPositions.length) {
                ...
                if (isDeclaration(node)) {
                    ...
                    if (!isString(declBetween)) {
                        throw new TypeError(
                            "The `between` property must be a string",
                        )
                    }
                    ...
                    for (const fixPosition of fixPositions) {
                        if (
                            fixPosition.startIndex <
                            declProp.length + declBetween.length
                        ) {
                ...
                if (isAtRule(node)) {
                    ...
                    if (!isString(atRuleAfterName)) {
                        throw new TypeError(
                            "The `afterName` property must be a string",
                        )
                    }
...
function inferDocIndentSize(document, space) {
    ...
    indentSize =
        Number(indentSize) ||
        (indents && indents[0] && indents[0].length) ||
        Number(space) ||
        2
...
function inferRootIndentLevel(root, baseIndentLevel, indentSize) {
    ...
    function getIndentLevel(indent) {
        ...
        const spaceCount = spaceMatch
            ? Math.round(spaceMatch.length / indentSize())
            : 0
    ...
    if (after) {
        ...
        if (after.endsWith("\n")) {
            ...
            if (document) {
                const nextRoot =
                    document.nodes[document.nodes.indexOf(root) + 1]

                afterEnd = nextRoot
                    ? nextRoot.raws.codeBefore
                    : document.raws.codeAfter
            } else {
    ...
    if (indents.length) {
        return (
            Math.max(...indents.map((indent) => getIndentLevel(indent))) +
            newBaseIndentLevel
        )
    }
