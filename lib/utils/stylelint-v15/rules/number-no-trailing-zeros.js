    ...
    return (root, result) => {
        ...
        function check(node, value) {
            ...
            valueParser(value).walk((valueNode) => {
                ...
                const index =
                    valueNode.sourceIndex + match.index + 1 + match[1].length
                ...
                const baseIndex = isAtRule(node)
                    ? atRuleParamIndex(node)
                    : declarationValueIndex(node)
            ...
            if (fixPositions.length) {
                for (const fixPosition of fixPositions) {
                    ...
                    if (isAtRule(node)) {
                        ...
                        node.value = removeTrailingZeros(
                            node.value,
                            startIndex,
                            endIndex,
                        )
