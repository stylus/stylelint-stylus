    ...
    return (root, result) => {
        ...
        function check(statement) {
            ...
            function nextNode(startNode) {
                if (!startNode || !startNode.next) return undefined

                if (startNode.type === "comment") {
                    const reNewLine = /\r?\n/
                    const newLineMatch = reNewLine.test(
                        startNode.raws.before || "",
                    )
                    ...
                    if (
                        next &&
                        newLineMatch &&
                        !reNewLine.test(next.raws.before || "")
                    ) {
                    ...
                    if (context.fix) {
                        ...
                        if (primary === "never-multi-line") {
                            ...
                            for (const [
                                node,
                                before,
                            ] of backupCommentNextBefores.entries()) {
                            ...
                            while (fixTarget) {
                                ...
                                if (
                                    reNewLine.test(fixTargetRaws.before || "")
                                ) {
                                    fixTargetRaws.before =
                                        fixTargetRaws.before.replace(
                                            /\r?\n/g,
                                            "",
                                        )
                                }
