...
function toRuleRow(rule) {
    ...
    const link = `[${rule.ruleName}](https://stylus.github.io/stylelint-plugin-stylus/rules/${rule.fileName}.html)`
...
function toDeprecatedRuleRow(rule) {
    ...
    const replacedBy = replacedRules
        .map(
            (name) =>
                `[stylus/${name}](https://stylus.github.io/stylelint-plugin-stylus/rules/${name}.html)`,
        )
        .join(", ")
...
${fs
    ...
    .replace(/\.\/docs\//gu, "./")
    .replace(
        /\(https:\/\/stylus.github.io\/stylelint-plugin-stylus\/(.*?)(\.html)?\)/gu,
